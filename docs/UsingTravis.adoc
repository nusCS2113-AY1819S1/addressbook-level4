...
.  On GitHub, create a new user account and give this account collaborator and admin access to the repo. +
   Using this account, generate a itemal access token https://github.com/settings/tokens/new[here].
+
[NOTE]
Itemal access tokens are like passwords so make sure you keep them secret! If the itemal access token is leaked, please delete it and generate a new one.
+
[NOTE]
...
* Click `Generate Token` and copy your new itemal access token.
--
...
--
* name: `GITHUB_TOKEN`
* value: itemal access token copied in step 1
...
Otherwise, other people will be able to see the itemal access token and thus have access this repo. +
...
.  Go to the URL `\https://<your-username-or-organization-name>.github.io/stocklist-level4/`. You should see your `README` file displayed.
