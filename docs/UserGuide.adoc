...
SMS is targeted mainly at university lectures as it allows them to easily organise and manage their students and classes with newly implemented group features.
...
Lecturers can further automate some of these processes by importing student data given by their department.
...
adds a contact named `John Doe` to the Student Management System.
...
* The index refers to the index number shown in the displayed *group* and *student* panels.
...
* Adding duplicate persons to a group is *not* allowed (i.e. _if the person already exixts in the group, adding that person to the group again is not allowed_).
* This command *is* undoable.
****
...
****
* Removes studnet at specifed INDEX on the *Students in Group* panel from the group at specified INDEX on the *Groups* panel.
