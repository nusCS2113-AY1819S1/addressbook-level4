...
kbd:[Space] or mouseclick anywhere else in the command box to move onto another word.
...
and date denotes the date the financial activity was completed. Each financial activity can be labelled with only 1
name, moneyflow and date and a maximum of 2 tags as these tags will be used to categorise each record to remove the need for user to specify another field.
...
** Tags will also only from from earlier saved records' tags and a pre-existing set of tags such as e.g.
food, travel, bills, shopping.

[NOTE]
* There is no autocompletion restriction on number of names, dates, moneyflow or tag inputs but command will still
require the appropriate number of parameters.
// end::autocomplete-b[]
...
* Performs similarly to the `add` function. Please refer to <<Add_Autocomplete>>.
// end::autocomplete-g[]
...
* There will be no suggestions on parameters that co
// end::autocomplete-i[]
...
image::Export_Capture1_LinhChi.png[width="350"]
