...
User: Shows event reminders for all upcoming events that the user has registered for. 
Format: `reminder`

IMPORTANT: Upcoming events are those that are occurring within the next 24 hours.
...
User: Updates the statuses of events. +
Format: `update`

IMPORTANT: Events that have already occurred will have the [COMPLETED] status. Events that have yet to occur will have the [UPCOMING] status.
...
* `export myCalendar` +
Exports the iCalendar file with name 'myCalendar' to your *src/data* folder
