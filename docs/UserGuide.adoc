...
* `login X1234567X zaq1xsw2cde3 member` +
Logs into Address book with user ID as X1234567X, password as zaq1xsw2cde3 and role as member.

image::CorrectIdPasswordRole.PNG[width="300"]
image::LoginSuccessMessage.PNG[width="300"]
...
* `createaccount X1234567X aq1xsw2cde3 member` +
Creates a new account with user ID as X1234567X, password as zaq1xsw2cde3 and role as member in the address book.
...
* The order of `\tag` and `\exclude` options cannot be swapped if you want to exclude tags from the displayed list.
* Back-to-back find commands will filter according to the displayed list. You can get more details about how it
works under the Search Pruning section
****
