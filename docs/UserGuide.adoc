...
Format: `reminder tt/TITLE d/DATE st/TIME ag/AGENDA`

[NOTE]
====
Reminders with the same `TITLE` are allowed but not with the same `DATE` *and* `TIME`
====

****
* Only 1 of each field is allowed.
* `TITLE`, `DATE`, `TIME` `AGENDA` all cannot be blank fields.
...
* `TIME` must be in 24 hours format, cannot exceed 2359 and only numbers will be accepted.
****
