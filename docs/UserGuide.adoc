...
.  Double-click on the jar file to start the app and wait for a few seconds for the loading to complete. You will then see a login pop up
.  window appears as shown in the image below. You will need to head over to the "Logging into system" section and follow the steps detailed 
.  carefully in order to log into the application successfully.
+
