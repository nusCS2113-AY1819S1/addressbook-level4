...
* Groups should be alphanumeric (contains letters and numbers) and can contain hyphens "-" and underscores "_"
****
...
+
.  After executing `group CS2113T`, all the users with the group `CS2113T` will be filtered and shown on the respective panels.
+
...
****
* See the example further below for a quick start!
* Command parameters:
** `FILE_NAME` refers to the name of the file that you want to import.
*** do *not* include the _.ics_ file extension.
...
** The file will be imported from the folder `import_export`, which is located in the same folder as the `FreeTime.jar` file. Please see the image below for a visual description: +
****
