...
Format: `add n/EVENT_NAME c/CONTACT_NAME p/PHONE_NUMBER e/EMAIL v/VENUE [t/TAG]...`
...
* `add n/Com Sci Workshop c/John Doe cp/98765432 e/johnd@example.com p/9876543 v/John street, block 123, #01-01` d/20/10/2017 10:30
* `add n/Sports Day c/Betsy Crow t/Sports e/betsycrowe@example.com v/COM2 #02-01 p/1234567 d/20/10/2017 10:30 t/leisure`
...
Format: `edit INDEX [n/EVENT] [c/CONTACT NAME] [p/PHONE] [e/EMAIL] [v/VENUE] [t/TAG]...`
...
* If you want to reset the comments section you can `C/{span}Comment Section{/span}{ol}{/ol}` as a field.
****
...
* When using different prefixes together, _logic AND operator_ is used. This mean if there is one prefix or morev in the combination that don't have any follow keywords, results search will always be zero.
****
...
Return any event having keyword `day` with name must have `Sports` and with date must have `12/01/2018`
// end::find[]
...
Format: `removeAttendee INDEX USERNAME`
...
Admin/User: Adds comment into event comment section with the username (when you are logged in) +
...
*  For `C/STRING`, `STRING` cannot be empty. `STRING` is the comment.
* Line has to be a non-zero integer and exist within the comment section. The numbers below the "Comment Section" in the BrowerPanel are the line parameters to index every comment. The comment will be replied under the line index given.
...
****
* Edits the event at the specified `INDEX`. The index refers to the index number shown in the displayed event list. The index *must be a positive integer* 1, 2, 3, ...
* Line has to be a non-zero integer and exist within the comment section. The numbers below the "Comment Section" in the BrowerPanel are the line parameters to index each comment. The line parameter provided will delete the corresponding comment index.
* Undo and Redo works! However, every time you undo or redo, the BrowserPanel will not refresh itself
  so you need to refresh it yourself.
****
...
* `export myCalendar` +
export the iCalendar file with name myCalendar to your *src/data* folder
...
* *Add* `add n/EVENT_NAME c/CONTACT_NAME p/PHONE_NUMBER e/EMAIL v/VENUE [t/TAG]...` +
e.g. `add n/Sports Day c/James Ho p/22224444 e/jamesho@example.com a/123, Clementi Rd, 1234665 t/friend t/colleague`
* *Clear* : `clear`
* *Delete* : `delete INDEX` +
e.g. `delete 3`
* *Edit* : `edit INDEX [n/EVENT] [p/PHONE_NUMBER] [e/EMAIL] [v/VENUE] [t/TAG]...` +
...
* *Remove Attendee* : `removeAttendee INDEX USERNAME` +
e.g. `removeAttendee 1 Peter Parker`
