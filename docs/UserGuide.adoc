...
Returns any person having the email `example@domain.com`
...
`t/TAG` flag can specified with a tag to open the email applications with all contacts belonging to that tag as recipients. +

If no flags are specified, selected contacts are chosen as recipients.

=== Text prediction

The command box is able to do smart prediction on the current text in the box. +
This is similar to text prediction on shell programs and can be invoked by pressing the `Tab` key on the keyboard. +
If there are more than one prediction, the predictions will be listed in results window. +

For example, typing `c` in the box and pressing `Tab` will auto complete to the `clear` command. +
Typing `e` in the box and pressing `Tab` will list `edit`, `exit` and `export` in results window. +

In general, text prediction works on all commands. +
In particular, text prediction also works on the following attributes of a contact: +

* `name` +

This feature is supported by the `find` command. When using `find`, first input the command, followed by any string of characters, then pressing `Tab` will invoke this feature. +

For example in an address book consisting of a contact with the name `Alex Yeoh`, typing `find A` and pressing `Tab` will auto complete to `find Alex Yeoh`. +
If no flags are given, by default, names are assumed to be predicted.
