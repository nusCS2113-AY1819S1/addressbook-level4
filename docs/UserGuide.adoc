...
Adds a task to the expenditure tracker. +
Format: `ET_add e/DESCRIPTION d/DATE m/MONEY c/CATEGORY`

****
* The format for `d/DATE` tag must be DD-MM-YY, where DD refers to the date, MM refers to the month and YY refers to the month in numbers.
* The m/MONEY only accepts integers which refers to the money spent on this.
* The c/CATEGORY only accepts strings: Food, Drink, Clothing, Electronics, DailyNecessities, Sports, Communications, Travels, Study, Office, Pets, Gifts, Entertainment, Traffic, Shopping, Beauty and Furniture.

****

Examples:

* `ET_add e/iPhoneX d/09-08-2018 m/1500 c/Electronics`
...
****
* Deletes the expenditrue at the specified `INDEX`.
* The index refers to the index number shown in the displayed expenditrue list.
* The index *must be a positive integer* 1, 2, 3, ...
****
...
****
* Edits the expenditrue at the specified `INDEX`. The index refers to the index number shown in the displayed expenditure tracker. The index *must be a positive integer* 1, 2, 3, ...
* At least one of the optional fields should be provided. There will not be any changes made if no optional fields are provided.
* Existing values will be updated to the input values.
****
...
* `ET_edit 2 e/ChickenRice ` +
Edits the expenditure name of the second expenditure be `ChickenRice`.
