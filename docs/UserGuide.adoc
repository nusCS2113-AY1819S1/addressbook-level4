...
* Currently, the only accepted parameters are `name` and `skill`.
****
...
* `sort st/skill` +
Sorts the list of people by name, in order of increasing skill level.

WARNING: The sort command currently sorts all data in NUSSU Connect. Interaction between this command and the `find`
command is coming in `v2.0`.
