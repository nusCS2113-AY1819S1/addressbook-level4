...
* `findj j/cashier` +
(Returns all `cashier` job offer)
* `findj c/KFC s/1500 h/SECONDARY` +
(Returns all job offers listed by `KFC` or job offers that has a pay of $`1500` or requires highest education level attained to be `SECONDARY` level)
...
(Returns any candidate with the name `alice` and phone number `91234567`)
...
(Returns any company having the name `HanBaoBao Pte Ltd` with address saved as `Yishun MRT`)
