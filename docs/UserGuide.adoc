...
[NOTE]
RecruitBook opens Company Book by default.
...
=== Adding a candidate / company / job  : `addc` `addC`
...
=== Selecting a candidate : `select`
Format: `select` `<INDEX>`

=== Selecting a company : `selectCompany`
Format: `selectCompany` `<INDEX>`

=== Selecting a job offer : `selectJob`
...
=== Exits from any intermediate command: `cancel`
...
* GUI (refer to Figure 4) : At the top of the application, locate the menu bar and click `File` > `exit`.
