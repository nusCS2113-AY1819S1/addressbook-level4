...
==== Find Student by name/matric no : `student find` (find by matric no coming in v1.3)
...
==== Find module : `module find [coming in v1.3]`
...
* Module code must exist
* http://Component[Component] name must not already exist in Trajectory.
...
NOTE: `*The optional parameter PRIORITY_LEVEL will be available from v1.4*` +
...
- The DATE parameter follows the `DD.MM.YYYY` formatting only. *(Validation coming in v1.3)*
...
* `note add m/CS2113 d/31.12.2113` +
This tells the system that the note is to be saved in the CS2113 module with a date (31 Dec 2113).

==== Edit note : `note edit [coming in v1.3]`
Edits an existing note. +
Format: `note edit INDEX m/MODULE_CODE [d/DATE]`
...
`note edit 3 m/CS1010 d/10.10.2019` +
The user will now be able to edit the note that corresponds to INDEX=3 in the list. The modified note will be saved with its assigned DATE changed to 10/10/2019.
...
==== Find note : `note find [coming in v1.3]`
Search and display existing notes using keywords. +
Format: `note find k/KEYWORD [k/KEYWORD]..`
...
NOTE: `*The optional parameter P will be available from v1.4*` +
