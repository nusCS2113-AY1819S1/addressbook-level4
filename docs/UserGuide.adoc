...
// tag::import[]
=== Import Timetable from an .ics file : `import` (`i`)

|===
*Use this command if you need to import your timetable from an external source, such as NUSMODS.*
|===

*Command details:* Imports a timetable from a specified file for the current user. It currently only supports files exported from NUSMODS. +

The files will be imported from the folder `import_export`, which is located in the same folder as the `FreeTime.jar` file. Please see the image below: +
...
* To import the timetable called `import.ics`, enter:
`import import.ics` +
...
// tag::export[]
=== Export Timetable as an .ics file : `export` (`e`)

|===
*Use this command if you need to export the currently-displayed timetable to an external application, such as Google Calendar.*
|===

*Command details:* Exports the currently-displayed timetable as an .ics file. +

The files will be exported to the folder `import_export`, which is located in the same folder as the `FreeTime.jar` file. Please see the image below: +
...
* To export the currently displayed timetable, type: `export export.ics` +
// end::export[]
