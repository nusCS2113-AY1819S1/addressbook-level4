...
* 1b. User's input parameters are missing or invalid. +
** Savee throws invalid command error and notifies user of the correct format.
** Use case ends.

* 1c. The record to be added is a duplicate of existing record in the system.
...
* 1b. The date entered is of incorrect format or invalid.
** Savee throws invalid command error and notifies user of the correct format.
** Use case resumes at step 1.
...
* 1b. User does not specify any arguments.
** Savee returns all records in the Savee.
** Use case ends.

* 1c. The arguments are of incorrect format
** Savee throws invalid command error and notifies user of the correct format.
** Use case resumes at step 1.
...
.  Savee requests for period of time of expenses to clear.
.  User enters period of time to clear expenses for.
.  Savee deletes all entries within specific time period.
+
Use case ends.
...
** Savee throws invalid input command
...
.  User requests to search for expenses by category
.  Savee requests for category of expenditure
.  User enters category desired
.  Savee displays each expenditure with name category and the total expenditure.
+
Use case ends.
...
.  Savee displays different commands
.  User inputs which command he wants to find out about
.  Financial displays information on selected command
+
Use case ends.
...
.  Savee confirms the undo request
.  User enter the item they would like to undo
.  Savee undo the deleted item
+
Use case ends.
...
.  User requests for adding notes
.  Financial Planner request for the date of entry and confirm the request
.  User enter the notes
+
Use case ends.
...
.  User requests to create daily limit for expenses
.  Savee requests for the maximum amount for the day
.  User enters the amount.
.  Savee confirms amount with user.
.  User confirms the amount.
.  Savee records the amount in the hard drive
+
Use case ends.
...
[discrete]
=== 12. Use case: Export record data from Savee to Excel file [Linh Chi]
...
[discrete]
==== 14. Use case: Import record data from Excel file to Savee. [Linh Chi]
...
[discrete]
=== 15. Use case: Archive the record data from Savee to Excel file. [Linh Chi]
...
.. Prerequisites: List all records using the `list` command. Multiple records in the list.
.. Test case: `add n/Linh Chi birthday gift d/31-03-1999 m/-50 t/friend` +
   `delete 31-3-1999` +
   Expected: If there are any record whose . Details of the deleted record shown in the status message. Timestamp in the status bar is updated.
.. Test case: `delete 0` +
   Expected: No record is deleted. Error details shown in the status message. Status bar remains the same.
.. Other incorrect delete commands to try: `delete`, `delete x` (where x is larger than the list size) +
   Expected: Similar to previous.
