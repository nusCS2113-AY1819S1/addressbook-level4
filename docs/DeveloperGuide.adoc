...
image::LoanListDiagram.png[width="800"]
...
The Tag feature is facilitated by `AddTagCommand`.
It extends `Command` and implements the following operations:

* `AddTagCommand()` --  Add new tags to a select item in the Stock List.

Given below is an example usage scenario and how the addTag mechanism behaves at each step.

Step 1. The user executes `addTag 1 t/ Lab2` command to add a tag `Lab2` to the item with index 2 in the Stock List.

Step 2. The 'addTag' command calls `updateFilteredItemList();` in `model` show the stock list after the tag is added.


==== Design Considerations

* **Alternative 1 (current choice):** Add tags to one item in the stock list.
** Pros: Easy to implement.
** Cons: May be complicated when the user want to add same tags to multiple items.
* **Alternative 2:** Add tag to multiple items in the stock list.
** Pros: Will be more user friendly when same tags need to be added to multiple items.
