...
image::StatisticParserActivityDiagram.png[width="800"]
...
The activity flow diagram below details the mechanism within the MixedPieChartDataList when its constructor is called.
...
** The first blue line named Series1 represents the Timeline (each component is one Date) versus the Income (each component is one income).
** The second orange line named Series2 represents the Timeline (each component is one Date) versus the Outcome (each component is one outcome).
** The third grey line named Series3 represents the Timeline (each component is one Date) versus the Net - Sum of income and outcome (each component is one net).
