...
** Cons: It needs effort to implement a `PopUpString` class (in our cases, a self-implemented pop-up window will be easier than using the one in Java library since only a string needs to be shown) and corresponding methods for UI activating in the `ExpenditureGetAdviceCommand` class.
