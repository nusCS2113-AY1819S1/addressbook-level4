...
In NUSSU Connect,
the main `SearchHistoryManger` object is in `ModelManager` and it stores `Predicate<Person>` objects which are then used to filter
`filteredPersons` list.

IMPORTANT: The `SearchHistoryManger` object in `ModelManager` only stores `Predicate<Person>` objects and is meant only for the filtering of `FilteredList<Person>`.
