...
The feature is facilitated by a few key components of FinancialPlanner, `Logic`, `Model`, `UI`. The corresponding command required for this feature is `stats`.
For this feature, users have to enter 2 dates, one starting date and one ending date. This execution of this feature can be split into 2 phases, the `Logic phase` and the `UI phase`.
