...
Step 3. The user executes `add name/David ...` to add a new person. The `add` command also calls `Model#commitModel()`, causing another modified state to be saved into every `...StateList`.
...
`Model#canUndo()` functions by checking whether the `currentStatePointer` of all 3 `Versioned` classes are at index 0.
...
** Pros: Will use less memory (e.g. for `delete`, just save the workout being deleted).
...
This method filters the entire workout list by `Calories`, `Difficulty` and `Duration` if specified by the user, and returns a new list of filtered workouts.
The original workout list is not modified. The activity diagram below depicts the execution flow of the `recommend` command.
...
For both paths, the filtered list returned from the `getFinalFilteredInternalList()` is passed to `WorkoutsPredicate` which compares every workout in the filtered list to the entire list of workouts.
...
.Interactions inside the `Logic` component for the `recommend mode/all calories/150` command.
image::SDForRecommendNonEvent.png[width="900"]

However, for single mode `recommend`, a new `JumpToRecommendListRequestEvent` is created and posted to `EventsCenter`.
...
Since the sequence is very similar to single mode recommend, the sequence diagram of this entire process can be seen in Figure 12,13 and 14.
...
==== Current implementation
The profile window is very similar to the help window. It gives the user a platform to store their personal information and workout presets. Once the MainApp is opened, it will load the `ProfileWindow.html` first.
...
image::ProfileCommand.PNG[width="500"]
...
image::ClickProfile.PNG[width="450"]
...
After parsing the file, the `ModifyCommandParser` will then find out which argument are to be edited. There will be CommandException thrown for any invalid or empty parameters. The parsed input will then specify which attribute are to be edited according to their prefixes.
After performing a boolean check to confirm which attribute have to be edited, the instance of `ProfileWindowManager` will then change the attribute by calling setAttribute, where Attribute can be calories, duration, gender etc..
...
.Class diagram that shows the how the different class interacts when executing the `modify` command
image::ModifyClassDiagram.png[width="500"]

.Sequence diagram that shows the Interactions inside the `Logic` component when executing `modify gender/male`.
image::ModifyCommandSD.PNG[width="650"]

[NOTE]
This modify feature only affects the `ProfileWindow.html` file and *will not* affect the workout book.
...
//tag::traversing[]
==== Current Implementation
The traversing mechanism is facilitated by `VersionedWorkoutBook`. It extends `WorkoutBook`.
...
// tag::memory[]
==== Current Implementation
The memory mechanism is facilitated by `VersionedWorkoutBook`. It extends `WorkoutBook`.
It implements following commands: +
...
Step 1. The user launches the application, `add` some workouts to the workout book and they will automatically have a `future` tag.

Step 2. The user executes the list command before the current command to change the `future` tag in the workout as specified by the index number to `current`.
There will also be a jump to the targetted workout according to the index number.
...
=== Tracking Feature
==== Current Implementation
The user input is parsed into `TrackCommandParser` to generate a `Parameter` object, which is then passed on to `TrackCommand`.
...
image::TrackFeatureActivityDiagram.png[width="950"]

==== Design Considerations
===== Aspect 1: How the workouts that match tracked parameters are stored

* **Alternative 1 (current choice):** All tracked workouts are stored in one file, `trackeddata.xml`
** Pros: Easy to implement storage of the tracked workouts.
** Cons: We have to implement a filtered list to allow for viewing of workouts containing the selected parameters after select INDEX is executed
...
** Cons: Difficult to specify the `Path` for each individual file, especially when the parameters in the `TrackedDataList` can vary widely during app usage. If one workout contains multiple parameters that are being tracked, it has to be stored multiple times across different files, reducing the effeciency of the program.
...
. Setting up parameters for the program to track
* Steps:
** The parameter to be tracked should not exist in the Tracked Parameters list.
** The value of the parameter to be tracked should be one word/string with no spaces, and match the restrictions of the prefix, e.g. if the prefix is `name/`, the value has to meet the restrictions for `Name`.
** Test Case: `track start muscle/quadriceps`
** Expected: `muscle/quadriceps` should appear on the Tracked Parameters list on the right-hand side of the program window.

. Stop tracking a parameter currently being tracked
* Steps:
** The parameter for which tracking is to be stopped should already exist in the Tracked Parameters list.
** Test Case: `track stop muscle/bicep`
** Expected: `muscle/bicep` should be removed from the Tracked Parameters list on the right-hand side of the program window.

. Viewing completed workouts containing a tracked parameter.
* Steps:
** There must be completed workouts that contain the desired parameter to be viewed. If not, no workouts will show up.
** Test Case: `select INDEX` (`INDEX` here refers to the index of the desired parameter)
** Expected: Completed workouts containing the parameter at `INDEX` will show up in the Tracked Workouts panel in the program.
...
.. Test case: `list` + `current 1` +
   ...
   There will also be a jump to the selected workout (1 in this case) according to the index number.
...
* Steps:
.. Test case: `list` + `current 1` (this workout or another workout already has a `current` state tag) +
   Expected: A message saying "There is already a current workout. Complete that before attempting again."
   There will also be a jump to the workout with the `current` state tag (1 if it already has the `current` state tag or another one with it).
