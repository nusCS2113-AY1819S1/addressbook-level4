...
*** The `AddExpensesCommand` will then check if any of the fields will contains negative value or exceed the limit. An exception will be
thrown if at least one of the fields contains negative value or exceed the limit.
...
*** The `AddExpensesCommand` will create an `expensesToEdit` object The `AddExpensesCommand` will then call
`createEditedExpenses` method with `expensesToEdit` and `EditExpensesDescriptor` object as parameters.
...
*** The `createEditedExpenses` method will call `modifyExpensesAmount`, `modifyTravelExpenses`, `modifyMedicalExpenses`,
`modifyMiscellaneousExpenses`, methods with `expensesToEdit` and `EditExpensesDescriptor` object as parameter.
...
*** Each method will edit their respective field from `expensesToEdit` with the same field from `EditExpensesDescriptor`.
...
----
*** The `AddExpensesCommand` will check for isNegativeLeftover and isOverLimit. If both are false, the
`AddExpensesCommand` will then update the updated expenses into CHRS.
...
image::Add_ExpensesCommandSequenceDiagram.png[width="900"]
...
Expected: Travel Expenses has value of 50.00. Medical Expenses has value of 100.00. Miscellaneous Expenses has value of 150.00. Total Expenses has value of 300.00. Timestamp in the status bar is updated. +
+
Test Case: addExpenses id/000001 tra/100 med/50 misc/25 +
...
Expected: Travel Expenses has value of 50.00. Medical Expenses has value of 100.00. Miscellaneous Expenses has value of 150.00. Total Expenses has value of 300.00. Timestamp in the status bar is updated. +
+
Test Case: addExpenses id/000001 tra/-20 med/-30 misc/-50 +
Expected: Travel expenses is decreased to 30.00, medical expenses is decreased to 70.00 and miscellaneous expenses is decreased to 100.00, total expenses is decreased to 200.00. Timestamp in the status bar is updated.
