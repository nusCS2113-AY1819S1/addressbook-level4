...
* `AddressBook#createGroup()` -- Creates a group in the address book.
* `AddressBook#hasGroup()` -- Checks to see if the address book has a particular group.
...
* `AddressBook#getGroupList()` -- Returns a list of groups in the address book.

These operations are exposed in the `Model` interface as `Model#createGroup()`, `Model#hasGroup()`, `Model#hasPersonInGroup()`, `Model#addGroup()` and `Model#getFilteredGroupList()` respectively.

Given below is an example usage scenario and how the `group` mechanism behaves at each step.

Step 1. User executes `group n/CS2113 l/LT15` command to add a group to the address book. The address book detects `group` as a command word will use `CreateGroupCommandParser` to
parse the input given. `CreateGroupCommandParser` will then create a `Group` object after parsing and pass it to `GroupCommand` which will then execute creating a group in the address book.
  +
[NOTE]
If a duplicate groups are found in the address book during its execution, it will not create a group or override the existing group but throw a `CommandException : Duplicate Group` which will notify the user of the issue.

Step 2. The user executes `addgroup g/1 p/1` command to add the first person in the address book to the first group in address book. The address book detects `addgroup` as a command word will use `AddGroupCommandParser` to
parse the input given. `AddGroupCommandParser` will then create a `AddGroup` object after parsing and pass it to `AddGroupCommand` which will then execute adding persons to a group in the address book. +



[NOTE]
If a duplicate persons are found in the group of the address book during its execution, it will not create a group or override the existing group but throw a `CommandException : Duplicate Persons` which will notify the user of the issue.

==== Design Considerations

===== Aspect: Create groups

* **Alternative 1 (current choice):** Creates a group and writes it to `addressbook.xml`.
** Pros: Easy to implement.
** Cons: Share same database as persons, not modular in terms of future expansions.
* **Alternative 2:** Creates a group and writes it to a different file `addressbook2.xml`.
** Pros: Modular as groups are stored in a different file, makes extraction of data easier.
** Cons: Harder to implement.

// end::group[]

// tag::AddScriptCommand[]
=== AddScriptCommand Feature
==== Current Implementation
The AddScriptCommand Feature is executed by AddressBook when the user enter the specific command.

Step 1: The User executes `AddScript StudentList`, the default constructor of the AddScriptCommand
will be executed and a new `Path` will be generated based on the parameters of the command. The path will be assign
with `StudentList.txt` for the above case.
// end::AddScriptCommand[]

// tag::distribute[]
=== Distribute Students Equally feature
==== Current Implementation
The Distribute Students Equally feature holds 4 different algorithms.
Each Algorithm serve different purposes.

* Distribute Students Equally
* Distribute Students Equally with Balanced Gender
* Distribute Students Equally with Balanced Nationality
* Distribute Students Equally with Balanced Gender & Nationality

Every Algorithm will require inputs such as:
****
* Number of Groups to be created of Integer Data Type
* 2D ArrayList of Person Type
* LinkList containing all person data from the current address book
* Group Name in String Type
****

From these 4 inputs, each algorithm will run necessary checks, create the required number of groups with the `Group Name` parsed in and add the Students assigned to the group.
...
. Place All Persons ArrayList into an LinkList.

===== Aspect: Distribute Students Equally Algorithm Explanation

*Allocation Process*

Firstly, I have created a 2D ArrayList of Person Type to allocate person to groups.

The 2D ArrayList act as a group which contains groups of selected persons.
Next, I have created an for loop to iterate _n_ times, where _n_ is the number of required groups to be created.
In the for loop, i have created an ArrayList of Person Type which stores all selected Persons.
In each for loop, the system will calculate the required number of person to be in a group and add them into the ArrayList.

Once the required number of person to be in a group is fulfilled, this ArrayList is stored into the 2D Array List.
This process loops for _n_ times.

The 2D Array List should now store _n_ Elements with (_a_) number of persons in each Element.

*Distribution Process*

Once the allocation process of person to groups. The `distributeProcess` method will access the 2D ArrayList.
In each ArrayList of the 2D ArrayList, the distributeProcess Method will create a group and add all the person allocated
to that ArrayList into the group.
This process loops until the 2D ArrayList if fully accessed.

*The process is now complete with all person distributed randomly into _n_ number of groups.*

===== Aspect: Distribute Students Equally with Balanced Gender Explanation

The process is similar to the Algorithm used above. However, the key difference is that I had separate all female person into a list
and all males into a list. After separation, I distribute 1 male person to each group and does this loop until there are no males left.

This process repeats for the female students.

Once the groupArrayList is allocated, we will run the same Distribution Process mentioned in the Algorithm above.

*The process is now complete with all person distributed randomly with balanced genders into _n_ number of groups.*
...
* `EmailCommand` -- The command to send an email to single or multiple indexes of persons.
...
===== Aspect: How to allow sending through different mail servers
...
|`* * *` |university lecturer |add groups to a calender |have a list of all my modules with their respective dates
...
[none]
* 1a. Student management system detects incorrect format of data entered.
+
...
2.  AddressBook confirms student is added
+
Use case ends
...
[none]
* 1a. Student management system detects incorrect format of in data entered.
+
...
1.  User requests to create N groups of students distributed via gender or nationality
2.  AddressBook confirms with user that N groups have been created
+
Use case ends
...
[none]
* 1b. AddressBook detects student count < N.
+

[none]
** 1b1. AddressBook outputs results using best-fit N value.
+
Use case ends
