...
Step 3. The user executes `add n/David ...` to add a new personc. The `add` command also calls `Model#commitAddressBook()`, causing another modified address book state to be saved into the `addressBookStateList`.
