...
* No Argument mode -- Requires no arguments and returns the entire list of records in Savee.
...
they are viewing *summary by date, by month or by category*. This feature involves most components ofSaveewith the exception of `Storage`. It can also be broadly split into
...
** Pros: Easier to implement and maintain. Sufficient for the intended target audience of Savee.
...
* Once `StatisticCommand#execute` is called, it will then search through the in-memory data of Savee and return a list containing all records within the date range and including
...
the event system in Savee.  The sequence diagram below details the program flow of the functions executed in `UI phase`.
...
It represents an in-memory model of theSaveeand is the component which manages the interactions between the commands and the `VersionedFinancialPlanner`.
...
==== Current implementation
The export into excel file mechanism is facilitated by `ModelManager` with the help of `ExcelUtil`, the utility created to handle all methods relating to Excel. It represents an in-memory model of theSaveeand is the component which manages the interactions between the commands, `ExcelUtil` and the `VersionedFinancialPlanner`.
...
==== Current implementation
The archive into excel file mechanism is facilitated by `ModelManager` with the help of `ExcelUtil`, the utility created to handle all methods relating to Excel. It represents an in-memory model of theSaveeand is the component which manages the interactions between the commands, `ExcelUtil` and the `VersionedFinancialPlanner`.
...
|`* * *` |user with many records in Savee |have an ordered list of records |locate records easily

|`* * *` |user with many records in Savee |find a record easily by name |access any record I want easily
...
[discrete]
=== 13. Use case: Create daily limit for expenses [Oscar]

*MSS*

1.  User requests to create daily limit for expenses
2.  Savee requests for the maximum amount for the day
3.  User enters the amount.
4.  Savee confirms amount with user.
5.  User confirms the amount.
6.  Savee records the amount in the hard drive
+
Use case ends.
...
** Savee throws invalid location error and prompts for new location.
** User enters a new location.
** Savee requests for confirmation.
** User confirms the location
** Use case ends.

[discrete]
=== 14. Use case: Export expenses into Excel file [Linh Chi]
...
[discrete]
==== 15. Use case: Import the Excel file into Financial Planner. [Linh Chi]
...
[discrete]
=== 16. Use case: Archive the records data into Excel file. [Linh Chi]
...
[discrete]
=== 17. Use case: See monthly financial activity
...
[discrete]
=== 18. Use case: Record parties to make payment to

*MSS*

1. User requests to add parties to make payment to.
2. Savee adds the person into the system.
