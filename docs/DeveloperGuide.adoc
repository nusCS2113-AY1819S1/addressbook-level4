...
The reminder mechanism is facilitated by the `ReminderComand` from the `Logic` component. A `Reminder` object is instantiated to add a reminder and each `Reminder` object consists of `Title`, `Date`, `Time` and `Agenda` objects.
