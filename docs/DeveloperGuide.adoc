...
* **Alternative 2:** Detects the difference between the currently displayed `TimeTable` before adding or deleting `TimeTablePanelTimeSlot` objects accordingly.
...
.Interactions between the Logic Component and Model Component for the `import` Command
...
* **Alternative 1 (current choice):** All code related to parsing .ics timetable files are inside 'IcsUtil' class.
...
=== Friend feature
==== Feature Description
The friend feature allows users to befriend and add other users to their friend list. Timetables are only viewable for users in your friend list and timetables can only be deconflicted within your friends.

==== Current implementation

This feature is implemented with two main parts. The front end and the backend implementation, which will be explained below.


===== Backend implementation

The `User` is associated with a specific `Person`. Each person can have any number of `Friend` objects. Every friend will have a `Name` object that corresponds to the specific `Person` object in the list.

.Class Diagram of the Friend class
...
.Frontend implementation of Friend feature
image::FriendFeatureFrontEndDiagram.png[width="300"]

===== Design Considerations
===== How to store the Friends that a user has.
* **Alternative 1 (current choice):** `Person` to compose `Friend` to compose `Name`
** Pros:
** Cons:
* Alternative 2: Allowing every `Friend` object compose a `Person` instead of a `Name`
** Pros:
** Cons:
// end::friend[]
