...
----
The above figure shows the sequence when the `AuthReturn` is `COMMAND_ALLOWED`. When it is not that, there will be an `SecurityAuthenticationException`
thrown with the relevant message to the user.

`CommandsEnum` is another enum type that contains iscommandAllowed(). They are found in `seedu.addressbook.commons` package.
