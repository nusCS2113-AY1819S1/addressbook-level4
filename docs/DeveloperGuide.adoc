= Timetable Deconflicter (TBC) - Developer Guide
...
* can save the `TimeTable` Objects in ics format and read it back.
* can read `TimeTable` data from a user-specified file in ics format.
...
The timetable feature allows users to store and view their own timetables. Additionally, it allows users to view a deconflicted timetable, a timetable which highlights mutual free timeslots among all selected users.
...
The `TimeTable` object is composed under the `Person` class in `Model`. Each `TimeTable` in turn is composed of any number of `TimeSlot` objects. Each `TimeSlot` consists of: +
1. A `dayofWeek` variable, denoting the day of week of the `TimeSlot`; +
2. A `startTime` variable, denoting the start time of the `TimeSlot`; and +
3. An `endTime` variable, denoting the end time of the `TimeSlot`.

The following class diagram summarises the relationship between the components of the TimeTable class:

image:TimeTableClassDiagram.png[width="400"]

{nbsp}

The `TimeTable` class implements three key methods: +
...
3. `TimeTable#deconflict()` - to create a deconflicted timetable from two or more `TimeTable` objects +
...
The following sequence diagrams show the sequence of significant method calls for the execution of the `TimeTable#addTimeSlot()` method. The execution of the `TimeTable#removeTimeSlot()` method is implemented in a similar manner.
...
** Pros: Easy to implement, only one method (`loadTimeTable`) required
** Cons: May suffer from performance degradation, especially when the `TimeTable` to be loaded contains many `TimeSlot` objects
* **Alternative 2:** Detects the difference between the currently displayed `TimeTable` and adds or deletes `TimeTablePanelTimeSlot` objects accordingly.
** Pros: Will reduce execution time of methods which update the displayed `TimeTable`
...
The import timetable feature allows users to import their timetables from NUSMODS, as an .ics file.

The timetable feature consists of two parts: +
1. The backend, which handles the reading of the .ics file and the creation of the timetable object. +
2. The frontend, which parses user input, and informs the user of success or failure.


===== Backend implementation

The file is read and parsed into a TimeTable by IcsUtil class.
...
The `IcsUtil` class implements three key utility methods for handling .ics files: +
1. `IcsUtil#getTimeTableFromFile()` - to obtain a `TimeTable`; accepts the `filePath` of the .ics file as parameter +


===== Frontend implementation
Same as the other commands, see <<Events-Driven-Nature,*`Events-Driven nature of the design`*>>.

==== Design Considerations
===== Aspect: Which Classes (and which system(s)) should parse the ics file format?
* **Alternative 1 (current choice):** All code related to parsing .ice timetable files are inside 'IcsUtil' class.
** Pros: All functions that have to do with .ics parsing is in 1 class.
** Cons: Single responsibility principle is broken;
* **Alternative 2:** Follow the existing implementation of 'Storage'; create an IcsTimeTableStorage class to access the ics file in hard disk, just like what is being done with 'preferences.json' and 'AddressBook.xml' files.
** Pros: Better follows Single Responsibility Principle: classes that concerns storage are in the storage subsystem;
** Cons: Current implementation favors 'permanent' files on the hard disk: Compared to the 'preferences.json' and 'AddressBook.xml' files, the .ics timetable file is single-use, and can have varying filePaths specified by user.
// end::timetable[]
...
1. CLI +
2. UI Prompt +
Inside the UI package, there are two new windows created, LoginWindow and RegistrationWindow. Whenever login or register
process is successful, it raises their respective SuccessEvent to the event bus. The preexisting UIManager is subscribed
to this events, and will display the content of the UI and closing the respective boxes.

===== Back-End Implmentation
...
|`* * *` | Student | Book a slot in my friend’s calendar | His schedule will automatically be blocked-out
...
|`* *` | Student | My meetings to start after a certain time | Not wake up early
|`* *` | Student | My meetings to end before a certain time | Make it home for dinner
|`* *` | Student | My meetings to last for more than a set duration | Complete the agenda for the meeting
...
|`* *` | Student | My meetings to only take place on certain days of the week | Minimise the number of days I need to go to school
