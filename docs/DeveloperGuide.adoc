...
The *_Architecture Diagram_* given above explains the high-level design of the App. Given below is a quick overview of each gradebookComponent.
...
* At app launch: Initializes the gradebookComponents in the correct sequence, and connects them up with each other.
* At shut down: Shuts down the gradebookComponents and invokes cleanup method where necessary.

<<Design-Commons,*`Commons`*>> represents a collection of classes used by multiple other gradebookComponents. Two of those classes play important roles at the architecture level.

* `EventsCenter` : This class (written using https://github.com/google/guava/wiki/EventBusExplained[Google's Event Bus library]) is used by gradebookComponents to communicate with other gradebookComponents using events (i.e. a form of _Event Driven_ design)
* `LogsCenter` : Used by many classes to write log messages to the App's log file.

The rest of the App consists of four gradebookComponents.
...
Each of the four gradebookComponents

* Defines its _API_ in an `interface` with the same name as the Component.
* Exposes its functionality using a `{Component Name}Manager` class.

For example, the `Logic` gradebookComponent (see the class diagram given below) defines it's API in the `Logic.java` interface and exposes its functionality using the `LogicManager.java` class.
...
The _Sequence Diagram_ below shows how the gradebookComponents interact for the scenario where the user issues the command `delete 1`.
...
[NOTE]
Note how the event is propagated through the `EventsCenter` to the `Storage` and `UI` without `Model` having to be coupled to either of them. This is an example of how this Event Driven approach helps us reduce direct coupling between gradebookComponents.

The sections below give more details of each gradebookComponent.

[[Design-Ui]]
=== UI gradebookComponent
...
The `UI` gradebookComponent uses JavaFx UI framework. The layout of these UI parts are defined in matching `.fxml` files that are in the `src/main/resources/view` folder. For example, the layout of the link:{repoURL}/src/main/java/seedu/address/ui/MainWindow.java[`MainWindow`] is specified in link:{repoURL}/src/main/resources/view/MainWindow.fxml[`MainWindow.fxml`]

The `UI` gradebookComponent,

* Executes user commands using the `Logic` gradebookComponent.
* Binds itself to some data in the `Model` so that the UI can auto-update when data in the `Model` change.
* Responds to events raised from various parts of the App and updates the UI accordingly.

[[Design-Logic]]
=== Logic gradebookComponent
...
Given below is the Sequence Diagram for interactions within the `Logic` gradebookComponent for the `execute("delete 1")` API call.
...
* does not depend on any of the other three gradebookComponents.
...
The `Storage` gradebookComponent,
...
Classes used by multiple gradebookComponents are in the `seedu.addressbook.commons` package.
...
.. _Unit tests_ that test the individual gradebookComponents. These are in `seedu.address.ui` package.
...
1. First, add small local-impact (i.e. the impact of the change does not go beyond the gradebookComponent) enhancements to one gradebookComponent at a time. Some suggestions are given in <<GetStartedProgramming-EachComponent>>.

2. Next, add a feature that touches multiple gradebookComponents to learn how to implement an end-to-end feature across all gradebookComponents. <<GetStartedProgramming-RemarkCommand>> explains how to go about adding such a feature.

[[GetStartedProgramming-EachComponent]]
=== Improving each gradebookComponent

Each individual exercise in this section is gradebookComponent-based (i.e. you would not need to modify the other gradebookComponents to get it to work).
...
[TIP]
Do take a look at <<Design-Logic>> before attempting to modify the `Logic` gradebookComponent.
...
[TIP]
Do take a look at <<Design-Model>> before attempting to modify the `Model` gradebookComponent.
...
[TIP]
Do take a look at <<Design-Ui>> before attempting to modify the `UI` gradebookComponent.
...
[TIP]
Do take a look at <<Design-Storage>> before attempting to modify the `Storage` gradebookComponent.
...
By creating this command, you will get a chance to learn how to implement a feature end-to-end, touching all major gradebookComponents of the app.
...
. Add `Remark` to model gradebookComponent (you can copy from link:{repoURL}/src/main/java/seedu/address/model/person/Address.java[`Address`], remove the regex and change the names accordingly).
...
|`* * *` |teacher |add grade gradebookComponents |differentiate the grade gradebookComponents in a module (E.g. mid term test, finals examination)
...
|`* *` |teacher |list all grade gradebookComponents |view all the grade gradebookComponents that I have added to my module
...
1. Teacher creates grade gradebookComponent.
...
* *a. At any time, teacher chooses to cancel creating a grade gradebookComponent.
+
...
* Grade gradebookComponent and corresponding module code must exist.
...
1. Teacher updates grade gradebookComponent.
2. System requests for confirmation by displaying input.
3. Teacher confirms request.
4. System indicates success message.
+
...
* *a. At any time, teacher chooses to cancel updating a grade gradebookComponent.
+
** *a1. System requests to confirm the cancellation.
** *a2. Teacher confirms request.
+
...
* Deleting grade gradebookComponent will also delete any grade items associated to it.

*MSS* :

1. Teacher removes grade gradebookComponent.
2. System requests for confirmation by displaying input.
3. Teacher confirms request.
4. System indicates success message.
+
...
* *a. At any time, teacher chooses to cancel removing a grade gradebookComponent.
+
** *a1. System requests to confirm the cancellation.
** *a2. Teacher confirms request.
+
...
1. Teacher finds grade gradebookComponent.
2. System displays details on selected grade gradebookComponent.
+
Use case ends.
...
* *a. At any time, teacher chooses to cancel finding grade gradebookComponent.
+
** *a1. System requests to confirm the cancellation.
** *a2. Teacher confirms request.
+
...
1. Teacher lists grade gradebookComponent.
...
* *a. At any time, teacher chooses to cancel listing a grade gradebookComponent.
+
** *a1. System requests to confirm the cancellation.
** *a2. Teacher confirms request.
+
