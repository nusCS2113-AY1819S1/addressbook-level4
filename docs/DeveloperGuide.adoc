...
// tag::addRecruitmentPost[]
=== Add Recruitment Post Feature
The command addRecruitmentPost allows HR staffs to add recruitment Posts based on open Job positions.

==== Current Implementation
The implementation of addRecruitmentPost command is supported by addRecruitmentPostCommandParser
that implements the parser interface. The interface parse the arguments parameter entered by
users to form the addRecruitmentPost object.

The implementation of addRecruitmentPost command checks that command without job position will be flagged
as invalid post.

Code snippet of addRecruitmentPostCommandParser:
[source, java]
----
...
==== Design Considerations
Additional consideration of addRecruitmentPost command:
* Check the validity input

* Usage of prefix to have more fields of input onto the posts.

// end::addRecruitmentPost[]
...
All logic commands comes with a dedicated parser. These parsers ensures that the input conforms to the following
...
.. Index parsed is a invalid index in the ScheduleList panel.

==== Add Works
`addWorks` command allows multiple work schedules to be added based on
the employees shown in the employee's panel.
Shown below is the normal flow of the sequence diagram for adding a working schedules.
...
the employees shown in the employee's panel.
Shown below is the normal flow of the sequence diagram for adding a leave schedules.
...
==== Current Implementation for `deleteExpenses` command
...
Apart from parsing the input, the `FilterCommandParser` checks whether the input complies with the required format.
This checking of input compliance is facilitated by two helper methods, namely `processDepartmentKeywords` and `processPositionKeywords`, that is within the class.

[NOTE]
The `processDepartmentKeywords` and `processPositionKeywords` methods are quite similar. Therefore, the code snippets will include only `processDepartmentKeywords` rather than both.

[NOTE]
If the input does not comply with the required format, an error message will be shown to the user.

Code snippet of `FilterCommandParser` that shows the checking of the input compliance:
...
The `processDepartmentKeywords` is further assisted by two other helper methods, namely, `validityCheckForDepartments` and `areDepartmentKeywordsValid` which are also within the `FilterCommandParser` class. The code snippet of the two mentioned methods are as follow:
...
`FilterCommand` first checks which prefix(es) is(are) present prior to calling the `updateFilteredPersonList` method, which updates `filteredPersonList`
 ...
 Additionally, the method `updateFilteredExpensesList` and `updateFilteredScheduleList` will be called to update the `filteredExpensesList` and `filteredScheduleList` respectively to show only the matched employees' expenses and schedules.
...
Example: If `filter asc d/hello` results in 0 person found, the CLI will list the currently available department(s) in CHRS.
...
Upon the update of `filteredPersonList`, `filteredExpensesList` and `filteredScheduleList`, the program will display only employees alongside their respective expenses and schedule that are of the specific department(s)
...
image::filterCommandSequenceDiagram.png[width="850"]
...
** Cons: Additional methods required to check prefix and validity of input
* **Alternative 2**: Separate commands for filter, one for department and one for position
** Pros: Easy to implement as it follows the existing `find` command
** Cons: More commands to create and implementation can get quite complicated when filter includes both fields
// end::filter[]
