...
===== Aspect: How the feature is implemented
...
** Cons: `ViewTaskCommand` class needs to handle three different cases, each with its own result and output.
* **Alternative 2:** Saves all variation of the filtered task list
...
1. User enters `TDL_sort by mod` command
...
1. User enters `TDL_sort by ddl` command
...
1. User enters `TDL_sort by impt` command
