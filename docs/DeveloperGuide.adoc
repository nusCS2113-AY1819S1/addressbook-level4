...
The `Security` component is a module that ensures the users that are using the application are
authenticated, and an instance of an authenticated user will be created and stored in `ModelManager` every time a user
is logged in so as to allow the application to do user-specific tasks.

It also handles all the commands entered to check for security clearance. On top of that, it is able to hash passwords and
save them into a file named users.json at root folder.

In Figure 9, AppUsers is  an object that stores the list of AccountCredentials. This object is created when _FreeTime_
runs. Storage Component will try to find a user.json file to convert that into the AppUsers object. If the file is not found, or the data is corrupted,
this program will create a new AppUsers object with default Users in it.
...
* can save `AppUsers` objects in json format and read it back.
...
1. `IcsUtil#readTimeTableFromFile()` - to read a `TimeTable` from an _.ics_ file; accepts the `filePath` of the _.ics_ file as parameter +
...
Model. This User is an extension of the Person Class, which different part of the UI and `SecurityManager` will access it. One example would
be the friend's UI panel, where _FreeTime_ has to know the current user to display his friends
...
1. CLI +
CommandBox Class handles the command line interface, and passes every command to
...
The Sequence Diagram when a user enters a command is seen below Figure 10:

.Interactions between the UI Component and Security Component for the login Command
image::SecurityParseCommandsSD.png[width="800"]

`AuthReturn` is a enum type that contains the following defintion:
----
COMMAND_ALLOWED, COMMAND_LOGOUTFIRST, COMMAND_LOGINFIRST, COMMAND_ERROR;
----
Figure 10 displays when the `AuthReturn` is `COMMAND_ALLOWED`. When it is not that, there will be an `SecurityAuthenticationException`
thrown with the relevant message to the user.
...
Figure 20 shows the steps when login is called after Figure 19 has completed (since login is a command too).
...
calls the static Hasher class, which takes the password and a salt to create the hash of the password.

Whenever a user logs in, the username is searched from userList returend by AppUsers. Once it matches a username, the input pasword is
then salted and hashed (SHA-512), and finally compared to the stored hashed password. A boolean is then returned to the
SecurityManager.
