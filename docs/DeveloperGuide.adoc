...
Step 1. The user launches the application, click on an event or types `select INDEX` into the CLI. The comment section will be seen as the LogicManager#execute("select").

Step 2. The user/admin executes `addComment 1 C/May I ask, what is the attire for the event?` to add a comment to the first event in the Event Manager. `AddCommentCommand` command obtains the comment section from `eventmanager.xml` calls `CommentFacade` to add comment "May I ask, what is the attire for the event", into the comment section and stores the comment section into `eventmanager.xml`. This is done by `LogicManager#execute("addComment 1 C/May i ask, what is the attire"`
