...
[NOTE]
Note how the `Model` simply raises a `FinancialPlannerChangedEvent` when the Address Book data are changed, instead of asking the `Storage` to save the updates to the hard disk.
