...
After forking the repo, the documentation will still have the SE-EDU branding and refer to the `se-edu/stocklist-level4` repo.

If you plan to develop this fork as a separate product (i.e. instead of contributing to `se-edu/stocklist-level4`), you should do the following:
...
[NOTE]
Note how the `Model` simply raises a `StockListChangedEvent` when the Address Book data are changed, instead of asking the `Storage` to save the updates to the hard disk.
...
[NOTE]
As a more OOP model, we can store a `Tag` list in `Address Book`, which `Item` can reference. This would allow `Address Book` to only require one `Tag` object per unique `Tag`, instead of each `Item` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
...
Classes used by multiple components are in the `seedu.stocklist.commons` package.
...
** See this https://github.com/se-edu/stocklist-level4/pull/785[PR] for the full solution.
****
...
** See this https://github.com/se-edu/stocklist-level4/pull/790[PR] for the full solution.
****
...
** See this https://github.com/se-edu/stocklist-level4/pull/798[PR] for the full solution.
...
** See this https://github.com/se-edu/stocklist-level4/pull/799[PR] for the full solution.
...
** Use link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter#handleStockListChangedEvent(StockListChangedEvent)`] to update the number of items whenever there are new changes to the stocklist.
...
** See this https://github.com/se-edu/stocklist-level4/pull/803[PR] for the full solution.
****
...
* Solution
** See this https://github.com/se-edu/stocklist-level4/pull/594[PR] for the full solution.
****
...
*Scenario:* You are a software maintainer for `stocklist`, as the former developer team has moved on to new projects. The current users of your application have a list of new feature requests that they hope the software will eventually have. The most popular request is to allow adding additional comments/notes about a particular contact, by providing a flexible `remark` field for each contact, rather than relying on tags alone. After designing the specification for the `remark` command, you are convinced that this feature is worth implementing. Your job is to implement the `remark` command.
...
See this https://github.com/se-edu/stocklist-level4/pull/599[PR] for the step-by-step solution.
