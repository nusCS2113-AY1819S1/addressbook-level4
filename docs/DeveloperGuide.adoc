...
.  The userInput is processed by a Natural Language Processor and the relevant data is parsed into its corresponding classes
...
library is instantiated to write all persons to the default file path `/data/pineapple.csv`. Currently, it implements
...
`/data/pineapple.csv`, and overwrites the file if an older version is available.
...
[NOTE]
The file path is defined in `outputFilepath`, and is hardcoded as `/data/pineapple.csv` for now.

[NOTE]
Any existing file named as `pineapple.csv` at the defined path will be overwritten.
