...
* The data file path must be manually set for now and will be developed in future to support user config.
...
An example of such structure can be visualised in the diagram below:
...
Step 2. `addressbook.xml` will be copied to the `.backup` folder with the name being saved as the number of milliseconds which has passed since epoch. +
