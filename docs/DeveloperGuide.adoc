...
* stores the Todo List and Expenditure Tracker data.
...
* can save the Todo List and Expenditure Tracker data in xml format and read it back.
...
Both commands are similiar in most areas, the key differences are in the respective `Command` classes.
This feature reuses the methods in the `Model` component used by the edit task command.
...
** Pros: Easy to implement with no modification to `Model` component.
** Cons: May have performance issues in terms of memory usage due to an additional `Task` object.
* **Alternative 2:** Create additional methods in `Model`
** Pros: Should use less memory.
** Cons: We must ensure that the implementation of each individual methods are correct.
// end::completeuncomplete[]

// tag::viewtask[]
=== Viewing Tasks feature
==== Current Implementation
