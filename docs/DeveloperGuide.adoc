...
The comments feature is facilitated by `Comments` class in the Logic/Comments folder. `AddComment`, `DeleteComment` and `ReplyComment` classes extend the `Comments` class. The `CommentFacade` class creates objects of `AddComment`, `DeleteComment` and `ReplyComment`. The features of the following classes are as such:
...
* `AddComment` -- Adds a new comment to the end of the comment section with the `addComment(String comment, String username)` function and inherits from `Comments` to reuse code.

* `DeleteComment` -- Deletes a comment given the line parameter in `deleteComment(int line)` function and inherits from `Comments` to reuse code.

* `ReplyComment` -- Replies a comment given the line parameter in `replyComment(String comment, int line, String username)` and inherits from `Comments` to reuse code.
...
The comment features use `AddCommentCommand`
