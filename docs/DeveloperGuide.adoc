...
predicate determined after the user inputs their login credentials.
...
Step 2. The user executes `login A1234567M zaq1xsw2cde3` command in the input box that matches an account in
`LoginBook`. `LogicManager` then calls `ParseCommand(login A1234567M zaq1xsw2cde3)` in `AddressBookParser`.

image::CorrectIdPassword.PNG[width="300"]

Step 3. `AddressBookParser` instantiates the `LoginUserIdPasswordCommandParser` object and simultaneously calls the `parse(args)` method, returning
an `LoginUserIdPasswordCommand` object with the user input, parsed, to `AddressBookParser` and `LogicManager`.

Step 4. `LogicManager` calls the `execute()` method in `LoginUserIdPasswordCommand`. Next, `LoginUserIdPasswordCommand`
...
image::ParseCorrectIdList.PNG[width="300"]

Step 6. `LoginUserIdPasswordCommand` calls `updateFilteredLoginDetailsList(updatedPasswordPredicate)` in `Model` with the
...
[NOTE]
In step 2, if the user executes `login A1234561M zaq1xsw2cde3` command instead, the application will continue with steps
...
image::WrongIdCorrectPassword.PNG[width="300"]
image::WrongIdOrPasswordList.PNG[width="300"]

[NOTE]
In step 2, if the user executes `login A1234567M zaq1xsw2cde4` command instead, the application will continue with steps
2 to 7, but instead of one account remaining at the end of the filtering process, there will be no account in the updated
list as shown in the image below.

image::CorrectIdWrongPassword.PNG[width="300"]
image::WrongIdOrPasswordList.PNG[width="300"]

[NOTE]
In step 2, if the user executes `login A1234561M zaq1xsw2cde4` command instead, the application will continue with steps
2 to 7, but instead of one account remaining at the end of the filtering process, there will be no account in the updated
list as shown in the image below.

image::WrongIdPassword.PNG[width="300"]
image::WrongIdOrPasswordList.PNG[width="300"]

[NOTE]
In all cases where the user inputs either the wrong ID or password or both, `showLoginError()` in `UserLoginException` will be
called which brings up a warning dialog box that informs the user of wrong input credentials.

image::LoginException.PNG[width="300"]
...
image::EnterBudgetCommand.png[width="800"]
...
image::ExecutedBudgetCommand.png[width="800"]

[NOTE]
In Step 4, had the user entered a club name that already existed in the address book, then a duplicate message would be shown
prompting the user to edit their entered command and try again. Execution of subsequent steps would be stopped until the
user entered a unique club name

image::DuplicateBudgetCommand.png[width="800"]

This is the sequence diagram for the full functioning of the `budget` command:
...
The main implementation behind SearchHistoryManager is a Stack Data Structure +
and the following 3 methods of `SearchHistoryManager` are exposed for your usage +
...
In the following diagrams, UP is the short-form for User Predicate which stores the logic specified by the user and SP is the short-form for System Predicate which stores the search logic for the system

NOTE: User Predicate and System Predicate are not actual Classes, they are simply called that way to help in your understanding.
'''
`executeNewSearch(Predicate<T> predicate)` +
...
Upon receiving a new `Predicate` defined by the user, SearchHistoryManager will simply push the new `Predicate` into `searchHistoryStack`
...
This method will remove the System Predicate from the top of the Stack. +
In the event that the Stack is already empty, `SearchHistoryManager` will throw `EmptyStackException`.
