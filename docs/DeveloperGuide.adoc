...
When the `free` command is executed, a `DeconflictTimeTable` object is instantiated with all `TimeSlot` objects in the user's `TimeTable`. Subsequently, the `TimeTable` objects of every `Person` that is passed as an argument is added to the `DeconflictTimeTable`.
...
image::UpdateTimeTableSequenceDiagram.png[width="800"]
