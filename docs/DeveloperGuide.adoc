...
The following activity diagram summarizes what happens when a user executes `sort` command:
...
image::blacklistSequenceDiagram.png[width="800"]
...
This is the simplified action diagram when the user executes the email command, assuming the user is only executing the `add`
