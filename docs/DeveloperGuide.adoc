...
These operations are exposed in the `Model` interface as `Model#commitCandidateBook()`, `Model#UndoRecruitBook()` and `Model#redoCandidateBook()` respectively.
...
These operations are exposed in the `Model` interface as `Model#commitRecruitBook()`, `Model#undoCompanyBook()`
...
Step 5. The user then decides to execute the command `listc`. Commands that do not modify the candidate book, such as `listc`, will usually not call `Model#commitCandidateBook()`, `Model#UndoRecruitBook()` or `Model#redoCandidateBook()`. Thus, the `candidateBookStateList` remains unchanged.
