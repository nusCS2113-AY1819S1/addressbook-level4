...
//zcstart
=== Class Add/List feature
==== Current Implementation

The add/list mechanism is facilitated by `ClassroomManager` supported by `StorageController`.
...
These operations are used in the `ClassAddCommand` class class under `ClassAddCommand#execute()`.
...
These operations are used in the `ClassListCommand` under `ClassListCommand#execute()`..
...
Step 3. The classroom list with the coressponding classroom information will be appended to the with the support of the `StringBuilder` and displayed as a message successfully.
