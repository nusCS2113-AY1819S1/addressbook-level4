...
* For the LoanListCommand, the execute method calls the ChangeStatusCommand to change the status from Ready to On_Loan. A XmlAdaptedLoanerDescription object is then created from the LoanerDescription and is added into the XmlAdaptedLoanList. This is then saved inside the storage.

image::LoanList.png[width="800"]
