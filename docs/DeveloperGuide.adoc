...
Thus, to check whether the event clashes with an employee's schedule, we iterate through the `ObservableList<Event>` and check whether the `Event` contains the employee's email in the `Attendee` and clashes with the selected event.
