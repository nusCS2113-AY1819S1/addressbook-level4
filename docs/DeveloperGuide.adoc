...
**  Find out which of the existing API methods in  link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] and link:{repoURL}/src/main/java/seedu/address/model/candidate/Person.java[`Person`] classes can be used to implement the tag removal logic. link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] allows you to update a candidate, and link:{repoURL}/src/main/java/seedu/address/model/candidate/Person.java[`Person`] allows you to update the tags.
* Solution
** Implement a `removeTag(Tag)` method in link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`]. Loop through each candidate, and remove the `tag` from each candidate.
...
* Solution
** Modify the constructor of link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] to take in the number of candidates when the application just started.
** Use link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter#handleAddressBookChangedEvent(AddressBookChangedEvent)`] to update the number of candidates whenever there are new changes to the addressbook.
...
We have to properly encapsulate the remark in our link:{repoURL}/src/main/java/seedu/address/model/candidate/Person.java[`Person`] class. Instead of just using a `String`, let's follow the conventional class structure that the codebase already uses by adding a `Remark` class.

**Main:**

. Add `Remark` to model component (you can copy from link:{repoURL}/src/main/java/seedu/address/model/candidate/Address.java[`Address`], remove the regex and change the names accordingly).
...
Now we have the `Remark` class, we need to actually use it inside link:{repoURL}/src/main/java/seedu/address/model/candidate/Person.java[`Person`].

**Main:**

. Add `getRemark()` in link:{repoURL}/src/main/java/seedu/address/model/candidate/Person.java[`Person`].
...
. Modify link:{repoURL}/src/main/java/seedu/address/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete your `candidateBook.xml` so that the application will load the sample data when you launch it.)
...
Since `Person` can now have a `Remark`, we should add a helper method to link:{repoURL}/src/test/java/seedu/address/testutil/PersonBuilder.java[`PersonBuilder`], so that users are able to create remarks when building a link:{repoURL}/src/main/java/seedu/address/model/candidate/Person.java[`Person`].
...
|`* * *` |Job agent |List offers and employees |Access these information readily

|`* * *` |Job agent |List job offers based on urgency |Prioritize job offers to fulfill

|`* * *` |Job agent |Email job candidates |Reach out to job seekers regarding relevant job offers

|`* * *` |Job agent |Filter a list of candidates based on certain criteria |See relevant candidates based on the criteria

|`* * *` |Job agent |Blacklist problematic candidates |Avoid assigning undesirable candidates to job

|`* * *` |Job agent |Sort jobs of different nature (such as internships, part-time jobs) |There are more options for employees to choose.

|`* * *` |Job agent |Find relevant details to a specific job candidate |Assign the most suitable job to him/her

|`* *` |User |Hide <<private-contact-detail,private contact details>> by default |Minimize chance of someone else seeing them by accident

|`* *`|Job agent|Shortlist the best candidates for a specific job offer|See the best available candidates

|`* *`|Job agent|Share profiles of prospective candidates with employers|Recommend employees to the respective companies

|`* *`|Job agent|Edit the details of listed candidates|Update changes of the details

|`* *`|Job agent|Delete candidate|

|`*`|Job agent|List the best employees for each sector|I can recommend them to potential employers

|`*` |User with many candidates in the address book |Sort candidates by name |Locate a candidate easily
|=======================================================================
...
** 3a2. Prompts user to enter a new entry or to exit add command
+
Use case resumes at step 3.
...
1.  User requests to edit details of a candidate
...
[none]
* 1a. The name of candidate to edit is invalid
+
[none]
...
1.  User requests to list job offers
2.  RecruitBook displays a list of all submitted job offers
3.  User requests to edit a job offer
4.  RecruitBook displays successful edit message
+
Use case ends.
...
[none]
* 4a. The index number requested for edit is invalid.
+
[none]
...
[none]
* 4b. The tags to edit are invalid
+
[none]
**  4b1.  RecruitBook displays an error message
+
Use case ends.
...
2.  RecruitBook prompts user to enter name of candidate to be deleted
3.  User enters name of candidate
