...
==== Current Implementation
The Lost and Found mechanism is facilitated by `LostCommand` and `FoundCommand`. These two commands extend `Command`. And two class: LostDescriptor and Found Descriptor are created as well. The 2 commands implement the following operations:
...
* **Alternative 1** (current choice) When executing lost or found command, update the quantity of the item in the StockList.
...
* **Alternative 2** Create a lost&found list to record the lost&found history.
