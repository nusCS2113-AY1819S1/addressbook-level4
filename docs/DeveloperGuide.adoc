...
* `AddressBook#getGroupList()` -- Returns a list of groups in the student management system.

These operations are exposed in the `Model` interface as `Model#createGroup()`, `Model#hasGroup()`,
`Model#hasPersonInGroup()`, `Model#addGroup()`, `Model#deleteGroup()` and `Model#getFilteredGroupList()` respectively.

Given below is an example usage scenario and how the group mechanism behaves at each step.

Step 1. User inputs `creategroup n/CS2113 l/LT15` to create a group in the student management system.
The `AddressBookParser` detects `creategroup` as a command word to use `CreateGroupCommandParser` to parse the input given.
`CreateGroupCommandParser` will then create a `Group` object after parsing and pass it to `GroupCommand` which will then
...
The following sequence diagram shows how the create group operation works:
...
to parse the input given. `AddGroupCommandParser` will then create an `AddGroup` object after parsing and pass it to `AddGroupCommand`
