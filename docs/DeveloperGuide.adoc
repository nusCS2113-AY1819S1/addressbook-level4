...
`find` can search for Name, Address, Date and Tags. If prefix is added, find can search for event that must contain that keyword in that specific field

==== Aspect: How to improve search
* **Alternative 1 (current choice):** Modify the predicate to display the event that Name, Date, Address, Tags contains one of the keywords
** Pros: Follow the current structure of `find` command, which mean can reuse current resources
** Cons: Predicate have limited ability to handle complexity needed in further enhancement
