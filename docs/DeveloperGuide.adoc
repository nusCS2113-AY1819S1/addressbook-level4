...
* The `Add Command` calls on the `Item Constructor` which creates the item object, and sets the status of all 50 Arduinos to `Ready` by default.

Step 2. The user executes the `ChangeStatusCommand` to change the status of 10 Arduinos from `Ready` to `Faulty`.

* The ChangeStatusCommandParser class is called and takes in the input string from the user, parses it into the ChangeStatusDescriptor, and returns a new ChangeStatusCommand.
...
* The `Model#commitStockList()` is called to save the updated stock list.
...
* The sorted items are stored as `SimpleItem` objects, which stores the just name the and quantity, as these are the only 2 relevant fields.
...
* The `LoanListCommand#execute()` is called. This calls the `ChangeStatusCommand#execute()` to change the status from Ready to On_Loan.
* The `LoanListCommand#updateLoanList()` is called. This creates an XmlAdaptedLoanerDescription object from the LoanerDescription, and is added into the XmlAdaptedLoanList.
* The `LoanListCommand#updateXmlLoanListFile()` is called to save the changes in storage.

The following sequence diagram shows how the LoanList operation works:
