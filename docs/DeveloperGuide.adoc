= Something Daily - Developer Guide
...
The _Sequence Diagram_ below shows how the components interact for the scenario where the user issues the command `TDL_delete 1`.

.Component interactions for `TDL_delete 1` command (part 1)
...
.Component interactions for `TDL_delete 1` command (part 2)
...
.Interactions Inside the Logic Component for the `TDL_delete 1` Command
...
// tag::completeuncomplete[]
=== Completing/Uncompleting a task feature
==== Current Implementation

Marking of a task as `completed` or `uncompleted` uses both `Logic` and `Model` components.
Both `TDL_complete` and `TDL_uncomplete` commands are similar in most areas as they are implemented based on the edit task command.
This feature also reuses the methods in the `Model` component used by the edit task command.
...
image::CompleteUncompleteTaskSequenceDiagram.png[width="800"]
...
// end::completeuncomplete[]

// tag::viewtask[]
=== Viewing filtered tasks feature
==== Current Implementation

Viewing a filtered task list uses both `Logic` and `Model` components.
This feature reuses the existing `Model#updateFilteredTaskList()` method to update the filter to be used.
...
image::ViewTaskSequenceDiagram.png[width="800"]
...
Sorting tasks in the to-do list uses both `Logic` and `Model` components.
Generally, the implementation of `TDL_sort date`, `TDL_sort priority` and `TDL_sort module` are quite similar,
and they use different comparators which are stored in `SortComparator`.

Firstly, after getting the input from user, `SortCommandParser` split command to get the parameter
and choose the corresponding sorting method. Then, we use `Collection.sort()` in `UniqueTaskList` to sort
...
image::SortTaskSequenceDiagram.png[width="800"]
...
* **In `Model` part**: We convert all deadline of task from `TaskDate` into `Date` type which can easily be processed with Java library.
Besides that, we set all deadline to 0am and assume all tasks listed in the to-do list are in the same year as the date in system.
We new a `Date` variable which initialized with the system date and compare it with the deadline of tasks.
* **In `UI` part**: We use `setStyle` method of `Label` class to change the font color of id, task name, module code
and deadline for distinguishing uncompleted tasks with different deadline.
...
** Pros: Easy to create, no external library or API is needed and pulling data from to-do list and expenditure tracker will be feasible as well.
...
** Cons: Need more effort to integrate with our application and pull task and expenditure data from storage.
...
** Pros: Easy to implement, there is no need for sorting functions.
** Cons: Need to check all the expenditures in the list with time complexity O(n) which is relatively slow.
* **Alternative 2:** We keep two expenditure lists sorted by date or category respectively.
** Pros: Faster to get the expenditures, we only need to find the first and last appearance of the date or category, all expenditures between start and end will be selected.
** Cons: Harder to implement, more storage are needed to maintain the two sorted lists.

// end::expenditureViewing[]
...
image::ExpenditureGetAdviceSequenceDiagram.png[width="800"]
...
** Pros: User friendly, the content of advice will be shown clearly, which is easy for user to see.
** Cons: Need effort to implement a `PopUpString` class (in our cases, a self-implemented pop-up window will be easier than using the one in Java library since only a string needs to be shown) and corresponding methods for UI activating in the `ExpenditureGetAdviceCommand` class.
* **Alternative 2:** We show the advice message in the command result box.
** Pros: Easy to be implemented.
** Cons: Not user friendly and harder for user to view in the command result box which is relatively small.

// end::getAdvice[]


// tag::checkexpenditure[]
=== Check Expenditure
==== Current Implementation

Check the total money spent in a specific period uses both Logic and Model components.
It also generate a chart to show each expenditure in this period on each date.
...
// end::checkexpenditure[]

// tag::dataencryption[]
=== Data Encryption
==== Proposed Implementation
_{Implementation of the data encryption feature will be introduced and explained in the near future.}_
...
|`* * *` |student |add a new task to the to-do list |

|`* * *` |student |edit a task on the to-do list |keep my tasks updated and correct

|`* * *` |student |delete a task on the to-do list |remove any entries that I no longer need

|`* * *` |student |archive completed tasks |have a history of all tasks completed earlier throughout the school semester

|`* * *` |student with multiple modules |tag each task with a module code |identify all tasks specific to a particular module

|`* * *` |student with multiple tasks |tag each task with a deadline |identify all tasks that are time sensitive

|`* * *` |student with multiple tasks |tag each task with different levels of importance/priority |identify all tasks that requires more attention

|`* * *` |student with multiple tasks |filter the todo list by completion status |obtain a list of tasks based on their completion status

|`* * *` |student |add a new expenditure to the expenditure tracker |

|`* * *` |student |edit an expenditure in the expenditure tracker |keep my expenditures updated and correct

|`* * *` |student |delete an expenditure in the expenditure tracker |remove any entries that I typed wrongly

|`* * *` |student who is not capable of managing expenditure |see and check where did I spend my money and how much did I spend for each expenditure with the corresponding date |better understand my spending habits

|`* * *` |student who wants to know how much money did I spend and how much income did I get for a certain period |see the amount of spendings and income respectively by keying in the starting date and end date of a particular period |

|`* *` |student with multiple tasks |sort the todo list by module |view all tasks specific to a particular module

|`* *` |student with multiple tasks |sort the todo list by deadline |view which task is due the earliest

|`* *` |student with multiple tasks |sort the todo list by importance/priority |view which tasks are more important than the other

|`* *` |student with tasks that may last for a long time |easily record and view my progress with progress bar |

|`* *` |student |delete all archived completed tasks after the semester is done | get ready for the next semester

|`* *` |student with difficulty in remembering all deadlines |get notified at a certain time before deadline based on my choice |submit my work on time

|`* *` |student who wants to know how to save my money |obtain advices from the Expenditure Tracker by checking the categories which I mainly spent my money on |control my spendings in a particular category

|`*` |student with multiple projects |have a separate project window |view all tasks specific only to projects

|`*` |student with multiple tasks to complete in a day |let the system automatically generate the order of tasks based on priority |start working on the tasks

|`*` |student who does not record their money spending regularly |notified by the Expenditure Tracker to note down the amount of money spent on food, snacks or other categories respectively everyday |
|=======================================================================
// end::userStories[]
...
// end::extraTaskUseCases[]

// tag::otherTaskUseCases[]
[discrete]
...
1.The color of one task turns red which mean this task is uncompleted and the deadline has passed already
2.The color of one task turns yellow which mean this task is uncompleted and the deadline will come within 7 days
...
// end::otherTaskUseCases[]

// tag::expenditureUseCases[]
[discrete]
=== Use case 15: Add expenditure / income
*System:*
Expenditure Tracker & Calendar
...
.  Command-line commands should be simple and easy to remember.
