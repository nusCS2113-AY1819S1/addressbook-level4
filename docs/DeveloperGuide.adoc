...
image::TodoClassDiagram.png[width="400"]

The following diagram illustrates how the Todo operation works:

image::TodoSequenceDiagram.png[width="800"]
...
Firstly, when a user completes entering a command (after pressing ENTER key), if the command typed is invalid, the system will suggest a similar command based on the edit distance.

The second instance would be when the user presses tab while typing the command halfway. The system will suggest commands based on the current prefix string.
//end::suggestion[]
