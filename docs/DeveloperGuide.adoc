...
//tag::suggestion[]
=== Suggestions Feature
Gives helpful suggestions to users on commands to type.

==== Current Implementation
There are two instances when suggestions are given.

Firstly, when a user completes entering a command (after pressing ENTER key), if the command typed is invalid, the system
will suggest a similar command based on the edit distance.

The second instance would be when the user presses tab while typing the command halfway. The system will suggest commands
based on the current prefix string.
//end::suggestion[]
...
The reminder mechanism is facilitated by the `ReminderCommand` from the `Logic` component. A `Reminder` object is instantiated to add a reminder and each `Reminder` object consists of `Title`, `Date`, `Time` and `Agenda` objects.

The following diagram illustrates the Reminder class:

image::ReminderClassDiagram.png[width="200"]

The following diagram illustrates how the Reminder operation works:
