...
FinancialPlanner is an application my team has created for the NUS module CS2113T on Software Engineering Principles and Object Oriented Programming.
This project lasted for 10 weeks and we were tasked to develop a desktop application for a particular group of users.
FinancialPlanner's main target audience are students who need help managing their finances and likes typing. In FinancialPlanner,
...
** `Reason for the enhancement` FinancialPlanner also allows users to tag each financial activity with a tag that they can define. These tags can be used for analysing financial activity.
