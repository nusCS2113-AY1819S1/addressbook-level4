...
** Created the storage components for events.
** Created the event class and its relevant components
...
class diagram for parser. Include new test case for `login` and `sort` command for Appendix F.
