...
_BookInventory_ is a *multi-platform* desktop application catered *primarily* for store owners to manage their book supplies. It includes features such as adding, deleting and requesting of books in the inventory. Users interact with _BookInventory_ using *Command Line Interface (CLI)*, and has a *Graphic User Interface (GUI)* created with JavaFX. This application was developed by a *team of four* students over a period of one semester with the original source code adapted from the Address Book (Level 4) project by SE-EDU.

== Summary of contributions

* *Major enhancement*: added *the ability to conveniently stock, sell and delete books* from the inventory
** What it does: users can now execute `stock`/`sell`/`delete` commands by selecting books with their unique ISBN. To facilitate typing of ISBN, users can cycle through existing ISBNs in the inventory by pressing kbd:[Tab].
