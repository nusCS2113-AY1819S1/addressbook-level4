...
This Project Portfolio documents my contributions to _BookInventory_. _BookInventory_ is a *multi-platform* desktop application catered *primarily* for store owners to manage their book supplies. It includes features such as adding, deleting and requesting of books. Users interact with _BookInventory_ using *Command Line Interface (CLI)*, and has a *Graphic User Interface (GUI)* created with JavaFX. This application was developed by a *team of four* students over a period of one semester with the original source code adapted from the Address Book (Level 4) project by SE-EDU.
