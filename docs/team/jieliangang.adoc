...
** What it does: allows the user to invite employees to events, remove employees from events, and view an employee's event schedule on a selected date, month or year which is displayed on the event list panel.
** Justification: This feature facilitates managing and scheduling of events among teams of employees who have different work schedules, and allow managers and employees to monitor and handle their event schedules easily.
** Highlights: This feature considers whether the event an employee is invited to *clashes* with the particular employee's work schedule, and *guarantees* that schedule conflict will not occur. The underlying model structure implemented for this enhancement is carefully designed and allows extension commands `viewmine` and `selectEvent` to be easily implemented.
...
*** Updated the `add` and `edit` command to distinguish different employees by their unique email address
...
*** Updated the Developer Guide to include implementation details, use case and user stories for `invite` and `select` command.
*** Updated the Model, UI and Logic class diagram in Developer Guide
** Community:
*** PRs reviewed (with non-trivial review comments): https://github.com/CS2113-AY1819S1-T16-2/main/pull/114[#114] https://github.com/CS2113-AY1819S1-T16-2/main/pull/122[#122]
*** Reported feature bugs discovered during system testing (Issue: https://github.com/CS2113-AY1819S1-T16-2/main/issues/125[#125] https://github.com/CS2113-AY1819S1-T16-2/main/issues/126[#126])
** Tools:
...
include::../DeveloperGuide.adoc[tag=usecases_invite]
