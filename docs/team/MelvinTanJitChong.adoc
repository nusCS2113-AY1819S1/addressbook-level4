...
** What it does: allows the user to login into the application with a specific role so that the user is only able to access role-specific features within the application.
...
Due to the fact that the user can only log into the application with their relevant role, it prevents any unauthorized access to other features that are only accessible for other roles.
