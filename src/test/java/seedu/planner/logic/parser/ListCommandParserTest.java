...
public class ListCommandParserTest {
    ...
    public void parse_nonEmptyArgWithoutDatePrefix_failure() {
        assertParseFailure(parser, " " + TYPICAL_START_DATE.getValue() + WHITESPACE
                + TYPICAL_END_DATE.getValue(), MESSAGE_INVALID_FORMAT);
    }
    ...
    public void parse_oneArgument_success() {
        assertParseSuccess(parser, " " + PREFIX_DATE + WHITESPACE
                + TYPICAL_START_DATE.getValue(), new ListCommand(TYPICAL_START_DATE, TYPICAL_START_DATE));
    }
    ...
    public void parse_twoArgumentWithDatePrefixInCorrectOrder_success() {
        ...
        assertParseSuccess(parser, " " + PREFIX_DATE + WHITESPACE + TYPICAL_START_DATE.getValue()
                + WHITESPACE + TYPICAL_END_DATE.getValue(), expectedListCommand);
        assertParseSuccess(parser, " " + PREFIX_DATE + WHITESPACE + TYPICAL_START_DATE.getValue()
                + WHITESPACE + TYPICAL_START_DATE.getValue(), new ListCommand(TYPICAL_START_DATE, TYPICAL_START_DATE));
    }

    @Test
    public void parse_twoArgumentWithDatePrefixInWrongOrder_failure() {
        assertParseFailure(parser, " " + TYPICAL_END_DATE.getValue() + WHITESPACE
                + TYPICAL_START_DATE.getValue(), MESSAGE_INVALID_FORMAT);
    }

    @Test
    public void parse_invalidArgumentWithDatePrefix_failure() {
        // More than 2 arguments are not allowed
        assertParseFailure(parser, " " + PREFIX_DATE + WHITESPACE + TYPICAL_START_DATE.getValue()
                + WHITESPACE + TYPICAL_END_DATE.getValue() + WHITESPACE
                + TYPICAL_END_DATE.getValue(), MESSAGE_INVALID_FORMAT);
        assertParseFailure(parser, " " + PREFIX_DATE + WHITESPACE + TYPICAL_START_DATE.getValue()
                + WHITESPACE + INVALID_DATE, Date.MESSAGE_DATE_CONSTRAINTS);
        assertParseFailure(parser, " " + PREFIX_DATE + WHITESPACE + INVALID_DATE + WHITESPACE
                + TYPICAL_END_DATE.getValue(), Date.MESSAGE_DATE_CONSTRAINTS);
        assertParseFailure(parser, " " + PREFIX_DATE + WHITESPACE + INVALID_DATE + WHITESPACE
