...
public class RecordTest {
    ...
    public void isSameRecord() {
        ...
        // different date parameter -> returns false
        Record editedIndo = new RecordBuilder(INDO).withDate(VALID_DATE_BOB).build();
        assertFalse(INDO.isSameRecord(editedIndo));

        // different moneyflow -> returns false
        editedIndo = new RecordBuilder(INDO).withMoneyFlow(VALID_MONEYFLOW_EXPENSE_BOB).build();
        assertFalse(INDO.isSameRecord(editedIndo));
        ...
        // different date and moneyflow -> returns false
        editedIndo = new RecordBuilder(INDO).withDate(VALID_DATE_BOB).withMoneyFlow(VALID_MONEYFLOW_EXPENSE_BOB)
                .build();
        assertFalse(INDO.isSameRecord(editedIndo));

        // different date and name -> returns false
        editedIndo = new RecordBuilder(INDO).withDate(VALID_DATE_BOB).withName(VALID_NAME_BOB).build();
        assertFalse(INDO.isSameRecord(editedIndo));

        // different moneyflow and name -> returns false
        editedIndo = new RecordBuilder(INDO).withMoneyFlow(VALID_MONEYFLOW_EXPENSE_BOB).withName(VALID_NAME_BOB)
                .build();
        assertFalse(INDO.isSameRecord(editedIndo));

        // same name, same date parameter, same money flow parameter, different tags -> returns true
