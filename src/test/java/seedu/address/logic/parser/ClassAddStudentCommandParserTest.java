...
/**
 * Provides a test for the class add student command parser
 */
public class ClassAddStudentCommandParserTest {
    private static ClassroomManager classroomManager;
    private ClassAddStudentCommandParser parser = new ClassAddStudentCommandParser();
    ...
    public void parse_allFieldsPresent_success() {
        ...
        assertParseSuccess(parser, CLASS_NAME_DESC_T16 + MODULE_CODE_DESC_CG1111 + MATRIC_DESC_MEGAN,
                new ClassAddStudentCommand(classroom.getClassName().getValue(), classroom.getModuleCode().moduleCode,
                        studentMegan.getMatricNo().matricNo));
    }

    @Test
    public void parse_missingCompulsoryField_failure() {
        String expectedMessage = String.format(MESSAGE_INVALID_COMMAND_FORMAT, ClassAddStudentCommand.MESSAGE_USAGE);
        // no parameters
        assertParseFailure(parser, ClassAddStudentCommand.COMMAND_WORD, expectedMessage);
    }
