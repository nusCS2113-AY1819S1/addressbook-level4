 ...
 */
@Ignore
public class HelpCommandSystemTest extends FinancialPlannerSystemTest {
    ...
    public void openHelpWindow() {
        ...
        getRecordListPanel().click();
        getMainMenu().openHelpWindowUsingAccelerator();
        assertHelpWindowOpen();

        getDetailedRecordCardPanel().click();
        getMainMenu().openHelpWindowUsingAccelerator();
        assertHelpWindowOpen();
        ...
        executeCommand(SelectCommand.COMMAND_WORD + " " + INDEX_FIRST_RECORD.getOneBased());
        assertEquals("", getCommandBox().getInput());
        assertCommandBoxShowsDefaultStyle();
        assertNotEquals(HelpCommand.SHOWING_HELP_MESSAGE, getResultDisplay().getText());
        assertNotEquals(getRecordListPanel().getSelectedCardIndex(), getDetailedRecordCardPanel());
        assertListMatching(getRecordListPanel(), getModel().getFilteredRecordList());

        // assert that the status bar too is updated correctly while the help window is open
        // note: the select command tested above does not update the status bar
        executeCommand(DeleteCommand.COMMAND_WORD + " " + INDEX_FIRST_RECORD.getOneBased());
