...
public class ProductListPanelTest extends GuiUnitTest {
    ...
    @Test
    public void display() {
        initUi(TYPICAL_PRODUCTS);

        for (int i = 0; i < TYPICAL_PRODUCTS.size(); i++) {
            personListPanelHandle.navigateToCard(TYPICAL_PRODUCTS.get(i));
            Product expectedProduct = TYPICAL_PRODUCTS.get(i);
            PersonCardHandle actualCard = personListPanelHandle.getPersonCardHandle(i);

            assertCardDisplaysPerson(expectedProduct, actualCard);
    ...
    public void performanceTest() throws Exception {
        ObservableList<Product> backingList = createBackingList(10000);

        assertTimeoutPreemptively(ofMillis(CARD_CREATION_AND_DELETION_TIMEOUT), () -> {
            initUi(backingList);
            guiRobot.interact(backingList::clear);
        }, "Creation and deletion of product cards exceeded time limit");
    }
